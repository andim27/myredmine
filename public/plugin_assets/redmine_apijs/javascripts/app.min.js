...
this.totals[i]=j;}}};this.showMedia=function(ev){var media={};if(typeof ev!=='string'){ev.preventDefault();if(ev.target.nodeName.toLowerCase()==='a'){media.id=ev.target.getAttribute('id');media.url=ev.target.getAttribute('href');media.conf=ev.target.querySelector('input').getAttribute('value').split('|');}
else{media.id=ev.target.parentNode.getAttribute('id');media.url=ev.target.parentNode.getAttribute('href');media.conf=ev.target.parentNode.querySelector('input').getAttribute('value').split('|');}
media.alt=(ev.target.nodeName.toLowerCase()==='img')?ev.target.getAttribute('alt'):'';media.classNames=document.getElementById(media.id.slice(0,media.id.lastIndexOf('.'))).getAttribute('class');media.album=parseInt(media.id.split('.')[1],10);media.number=parseInt(media.id.split('.')[2],10);}
else{media.id=ev;media.url=document.getElementById(media.id).getAttribute('href');media.conf=document.getElementById(media.id).querySelector('input').getAttribute('value').split('|');media.alt=document.getElementById(media.id).querySelector('img');media.alt=(media.alt)?media.alt.getAttribute('alt'):'';media.classNames=document.getElementById(media.id.slice(0,media.id.lastIndexOf('.'))).getAttribute('class');media.album=parseInt(media.id.split('.')[1],10);media.number=parseInt(media.id.split('.')[2],10);}
