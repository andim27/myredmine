...
class User < Principal
  ...
  #--b:andreyM
  def self.get_group_name(user_id)
    res=find_by_sql("SELECT u.lastname FROM users as u WHERE u.id=(SELECT group_id FROM groups_users WHERE user_id=#{user_id} LIMIT 1) LIMIT 1")
    res[0].lastname
  rescue
  "?"
  end
